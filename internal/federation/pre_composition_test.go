...
func TestPreCompositionValidators(t *testing.T) {
	t.Parallel()

	type Spec struct {
		Name string
		Rule func(definition *ServiceDefinition) []error
	}

	specs := []*Spec{
		{
			Name: "externalUsedOnBase",
			Rule: externalUsedOnBase,
		},
		{
			Name: "requiresUsedOnBase",
	...
	for _, spec := range specs {
		...
		t.Run(spec.Name, func(t *testing.T) {
			...
			testFileDir := fmt.Sprintf("./_testdata/preCompositionValidators/%s/assets", spec.Name)
			expectFileDir := fmt.Sprintf("./_testdata/preCompositionValidators/%s/expected", spec.Name)
