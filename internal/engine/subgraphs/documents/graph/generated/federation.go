...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) []fedruntime.Entity {
	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		switch typeName {
		case "Noop":
			resolverName, err := entityResolverNameForNoop(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "Noop": %w`, err)
			}
			switch resolverName {

			case "findNoopByNoop":
				id0, err := ec.unmarshalOString2ᚖstring(ctx, rep["noop"])
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findNoopByNoop(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindNoopByNoop(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "Noop": %w`, err)
				}
...
func entityResolverNameForNoop(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
		...
		return "findNoopByNoop", nil
	}
	return "", fmt.Errorf("%w for Noop", ErrTypeNotFound)
}
